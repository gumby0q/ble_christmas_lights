...
static uint32_t pwm_level_char_add(ble_pwm_t * p_pwm, const ble_pwm_init_t * p_pwm_init)
{
    ...
    BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_PWM_UUID_VALUE_CHAR);
//    BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_UUID_BATTERY_LEVEL_CHAR);
    ...
    initial_pwm_level = p_pwm_init->initial_pwm_level;
    ...
    err_code = sd_ble_gatts_characteristic_add(p_pwm->service_handle, &char_md,
                                               &attr_char_value,
    ...
    if (p_pwm_init->p_report_ref != NULL)
    {   
        BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_PWM_UUID_COM_CHAR);
        // Add Report Reference descriptor
        //BLE_UUID_BLE_ASSIGN(ble_uuid, BLE_UUID_REPORT_REF_DESCR);

        memset(&attr_md, 0, sizeof(attr_md));

        attr_md.read_perm = p_pwm_init->pwm_level_report_read_perm;
        ...
        init_len = ble_srv_report_ref_encode(encoded_report_ref, p_pwm_init->p_report_ref);
        ...
        err_code = sd_ble_gatts_descriptor_add(p_pwm->pwm_level_handles.value_handle,
                                               &attr_char_value,
                                               &p_pwm->report_ref_handle);
        if (err_code != NRF_SUCCESS)
        {
            return err_code;
        }
    ...
    {
        p_pwm->report_ref_handle = BLE_GATT_HANDLE_INVALID;
    }
...
//    {
        ...
        if (err_code == NRF_SUCCESS)
        {
            ...
            NRF_LOG_INFO("ahahah %d", pwm_level);
        }
        else
        {
            return err_code;
